...
class breakdown extends Component {
  ...
  componentDidMount = () => {
    {this.props.buycharger!=""?this.document1():this.document()}
    // this.document();
...
// console.log('redux',props);
    let {chargingPointData, filePath,ser} = this.state;
...
data.append('service_status_type',2);
// console.log('audio',this.state.audio);
if(this.state.audio==""){
 
}else{
  // this.setState({isLoading:true})
  console.log('audio',this.state.isLoading);
  data.append('file', {
      uri:
        Platform.OS === 'android'
          ? this.state.audio
...
  });
  ...
  render() {
    ...
    let { isLoading, errors = {}, recording,image1, document_type, playTime, filePath, refId, recordTime,customer_doc_no, customer_contact, customer_doc_type, repeatPass, duration } = this.state;
