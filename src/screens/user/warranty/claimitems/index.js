...
class UserClaims extends Component {
    ...
    Chargerservice = () => {
      ...
      console.log(this.props,'Chargerservice')
      ...
      HttpRequest.charger_user_soldrecords({user_contact:props.info.phone})
          .then(res => {
              ...
              if (res.status == 200 ) {
                  ...
                  this.Chargerservice1(this.props.route.params);
              } else {
  ...
  Chargerservice1 = (data) => {
    ...
    getChargingStations1 = (data) => {
      // let { items } = this.props.route.params;
      let { radius, region,list } = this.state;
      console.log(data,'region')
    
      HttpRequest.dealer_warranty({stock_id:data})
          .then(res => { 
              const result = res.data;
                ...
                //  this.setState({ chargingPointData: result.data, refreshing: false,favorite: result.favorite, isLoading: false})
                 ...
                 list1.push({title:"Battery",name:result.battery_warranty_message,icon:Images.battery});
                  ...
                  this.state.list.forEach(name=>{
                    // console.log('out',name.title)
                    this.setState({distance:name.title})
...
onSubmit(){
  ...
  {this.props.buycharger==""?navigation.navigate("WarrantyPolicy"):navigation.navigate("UserWarrantyPolicy");}
...
itemDimension={120}
data={list}
renderItem={distance==""?this.renderItem:this.renderItem1}
key={(item)=>item.id+''}
