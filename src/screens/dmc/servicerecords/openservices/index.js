...
class openservices extends Component {
    ...
    getChargingStations = () => {
        ...
        HttpRequest.serive_open({distributor_code:props.info.phone})
            .then(res => {
                
                const result = res.data;
                console.log("Service open Response ---------- ", result);
                if (res.status == 200 ) {
  ...
  invoice(item){
      ...
      navigation.navigate('Serviceinvoice', { item });
...
export default connect(mapStateToProps,mapDispatchToProps)(openservices);
