...
const amd=[];
class closewarranty extends Component {
    constructor(props){
        super(props);
    ...
    getChargingStations = () => {
        ...
        HttpRequest.warranty_close({distributor_code:props.info.phone})
            .then(res => {
                
                const result = res.data;
                console.log("Warranty close Response ---------- ", result);
                if (res.status == 200 ) {
  ...
  invoice(item){
      ...
      navigation.navigate('Serviceinvoice', { item ,type});
    ...
    <View style={styles.item} activeOpacity={.7} >
       <View style={styles.itemHeader}>
            ...
            <View style={styles.row}>
                <Image source={{ uri: item.warranty_icon}} style={styles.image} />
                <View style={[styles.content, { flex: 3 }]}>
                    ...
                    <View style={[styles.row, styles.descriptionContent, { marginTop: 0 }]}>
                        <Text style={[styles.text, styles.description, styles.label]}>Warranty Name</Text>
                        <Text style={[styles.text, styles.description, styles.leftAlign]} numberOfLines={1}>{item.warranty_name}</Text>
                    </View>
                    ...
                    <View style={[styles.row, styles.descriptionContent]}>
                        ...
                        <Text style={[styles.text, styles.description, styles.leftAlign]}>{item.warranty_open_date}</Text>
                    </View>

                    <View style={[styles.row, styles.descriptionContent]}>
                        <Text style={[styles.text, styles.description, styles.label]}>Closed Date</Text>
                        <Text style={[styles.text, styles.description, styles.leftAlign]}>{item.warranty_close_date}</Text>
                    </View>
                    <View style={[styles.row, styles.descriptionContent]}>
                        <Text style={[styles.text, styles.description, styles.label]}>Status</Text>
                        <Text style={[styles.text, styles.description, styles.leftAlign]}>{item.warranty_status}</Text>
                    </View>
                    
                     <View style={[styles.row, styles.descriptionContent]}>
                        <Text style={[styles.text, styles.description, styles.label]}>Tracking Name</Text>
                        <Text style={[styles.text, styles.description, styles.leftAlign]} numberOfLines={1}>{item.tracking_name}</Text>
                    </View>
                    <View style={[styles.row, styles.descriptionContent]}>
                        <Text style={[styles.text,styles.description, styles.label]}>Tracking No.</Text>
                        <Text style={[styles.text,styles.description, styles.leftAlign]}>{item.tracking_no}</Text>
                    </View>
                    {/* <View style={[styles.row, styles.descriptionContent]}>
    ...
    return (
        <View style={styles.container}>
            <View  style={styles.header}>
              <Header navigation={navigation} type={strings.dmc.c_list}/>
            </View>
            {/* {orders != 0 ?
...
export default connect(mapStateToProps,mapDispatchToProps)(closewarranty);
