...
checkeRfid = (i) => {
  ...
  HttpRequest.checkecar({formData:data},this.props.token)
  .then(res => {
      this.setState({ isLoading: false });
      const result = res.data;
      console.log('carsucc',result);
      if(i<=9)
      {

        if (res.status == 200 && !result.error) {
        this.props.navigation.navigate("Carsuccess");
      ...
      <View style={styles.container}>
          <View  style={styles.header}>
           <View style={styles.center}>
                                     <Text style={styles.title}>{strings.cars.title}</Text>
                                   </View>
          </View>
          <Animatable.View animation="fadeInUpBig" style={styles.footer}>
             ...
             {({ remainingTime, color }) => (
              ...
              <Text style={styles.text} numberline={2}> Please connect your EV with the charger and wait till time out.</Text>
                    </View>
                </Animatable.View>
            </View>
...
export default connect(mapStateToProps,mapDispatchToProps)(CarId);