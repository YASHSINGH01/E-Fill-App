...
const amd=[];
class fProducthistory extends Component {
    constructor(props){
        super(props);
    ...
    getChargingStations = () => {
        ...
        HttpRequest.franchisee_soldrecords({distributor_code:props.info.phone})
            .then(res => {
                ...
                if (res.status == 200 ) {
                    ...
                    //  console.log("distance",this.state.chargingPointData[1].part_detail);
                } else {
...
console.log('sodlfran',data);
 this.props.navigation.navigate("fSoldrecords",{item:data});
  }

sendinvoice(text,chargePointSerialNumber){
    console.log('sodl',text,chargePointSerialNumber);
    HttpRequest.updateIc({charger_id:chargePointSerialNumber,email_id:text})
    .then(res =>{
        const result=res.data;
        if(res.status==200 && !result.error){
           ...
           // }
           Alert.alert(
              "I & C form has been sent to your maild ID"  
             );
       ...
       prompt(
        ...
        [
          ...
          },
          {
            ...
            onPress: (password) =>this.sendinvoice(password,item.part_detail.chargePointSerialNumber)
          }
...
nav(items){
        ...
        this.props.navigation.navigate('FranWarrantyClaims',{items});
}
    renderItem = ({ item }) => (
                ...
                <View style={styles.row}>
                    ...
                    <View style={[styles.content, { flex: 3 }]}>
                        ...
                        <View style={[styles.row, styles.descriptionContent, { marginTop: 10 }]}>
                            <Text style={[styles.text, styles.description, styles.label]}>Charger Serial No.</Text>
                            <Text style={[styles.text, styles.description, styles.leftAlign]} numberOfLines={1}>{item.part_detail.chargePointSerialNumber}</Text>
                        </View>
                        ...
                        :
                        <View style={[styles.row, styles.descriptionContent]}>
                          <Text style={[styles.text,styles.description, styles.label]}>Temparory RC no.</Text>
                          <Text style={[styles.text,styles.description, styles.leftAlign]}numberOfLines={2}>{item.customer.temp_rc_no}</Text>
                          </View>
} */}
 <View style={[styles.row, styles.descriptionContent ]}>
                ...
                <TouchableRipple onPress={() => this.handleEmail(item)} style={{flex: 1}}  rippleColor="rgba(255, 255, 255, .32)" activeOpacity={.7}>
                  <LinearGradient start={{x: 0, y: 0}} end={{x: 1, y: 0}} colors={['#FF6A00', '#EE0979']} style={styles.signInButtons}>
                    <Text style={styles.buttonTexts}>Update I & C</Text>
                  </LinearGradient>
              </TouchableRipple>
              :null}
...
export default connect(mapStateToProps,mapDispatchToProps)(fProducthistory);
