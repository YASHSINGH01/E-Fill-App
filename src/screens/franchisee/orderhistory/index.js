...
const amd=[];
class franchiseeOrder extends Component {
    constructor(props){
        super(props);
    ...
    getChargingStations = () => {
        ...
        HttpRequest.franchisee_orderhistory({distributor_code:props.info.phone})
            .then(res => {
                
                const result = res.data;
                console.log("Charger  Order history API Response ---------- ", result.data);
                if (res.status == 200 ) {
    ...
    navigate(item){
        ...
        this.props.navigation.navigate('f_detailsOrder',{item});
    }
    upload(item){
        console.log('navi',item)
        this.props.navigation.navigate('Payemnt',{item,role:'franchisee'});
    }
...
export default connect(mapStateToProps,mapDispatchToProps)(franchiseeOrder);
