...
class fServiceinvoice extends Component {
    constructor(props) {
        super(props);
    ...
    submit=(item) =>{
      // let { item } = this.props.route.params;
        ...
        //  console.log('jfhf',item.service_id,detail);
         HttpRequest.linkform({service_id:item.service_id,email_id:detail}).then(res=>{
        ...
        // this.props.navigation.navigate("fotpcancel", { mobile: item.customer_contact, service_id: item.service_id, service_response: detail,services:document_type });
    }

    service() {
        let { item } = this.props.route.params;
        console.log('hhyd', item);
        HttpRequest.franchisee_service()
          .then(res => {
...
:null}
 {item.service_status == 0?
                            <View style={styles.inputContainer}>
                                ...
                                <TextInput
                                    ref={this.phoneRef}
                                    placeholder="Enter service person  Email ID"
...
:null}
                        ...
                        {item.service_status == 0 ?
                            <TouchableOpacity style={styles.signInButton} onPress={() => this.submit(item)}>
                                <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} colors={['#FF6A00', '#EE0979']} style={styles.linearGradient}>
                                    <Text style={styles.cancelText}>Share Service Link</Text>
                                </LinearGradient>
                            </TouchableOpacity>
                            : null}
                    </ScrollView>
...
export default connect(mapStateToProps, mapDispatchToProps)(fServiceinvoice);
