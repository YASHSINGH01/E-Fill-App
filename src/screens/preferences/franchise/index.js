...
import { franchisers } from '../../../Redux/Actions/Actions';
...
class Franchise extends Component {
    ...
    componentDidMount = () => {
        ...
        this._checkNotification = this.props.navigation.addListener('focus', () => {
            AsyncStorage. getFranchise_passkey().then(val => {
    ...
    uniquetext() {
        ...
        AsyncStorage.setFranchise_passkey(subscription, subscribe);
         console.log('subscribed0', subscription);
        // this.props.franchisers(subscription, subscribe);
        
        console.log('subscribed', this.props.franchisers);

    }
    unique = (input) => {
        console.log(input);
        let { subscribe, subscription } = this.state;
        HttpRequest.franchise({ passkey: input })
            .then(res => {
                ...
                if (res.status == 200 && !result.error) {
                    ...
                    subscription = 0;
                    this.setState({subscription: subscription});
                    AsyncStorage.setFranchise_passkey(subscription);
                    this.props.franchisers(subscription);
                    Alert.alert(
                        "",
                        result.message,
                        [
        ...
        return (
            <View style={styles.container}>
                <View style={styles.header}>
                    <Header navigation={navigation} type={strings.preferences.franchise} />
                </View>
                    ...
                    <FlatList
                        ...
                        ListHeaderComponent={
                            <View style={styles.Intro}>
                                <Text style={[styles.introTitle]}>{strings.preferences.enable}</Text>
                            </View>
                        }
                    />
                </Animatable.View>
...
const mapDispatchToProps = (dispatch) => {
    return {
        franchisers: bindActionCreators(franchisers, dispatch),
    };
}
